...
$post.='</strong><br>'.'
        ...
        $post.='</em></small><hr> <p class="description">
...
function __construct($region,$limitrss)
{
  ...
  $region= $this->convertanddata($region);
    ...
    if($dbcach==null){
        echo 'still there';
        $dbcach=$db->fetchmyArray('select newsdetails.name,country,url_RSS,url from newsdetails, newsreference  where region in ('.$region.') and newsdetails.name=newsreference.name order by statistics desc;');
