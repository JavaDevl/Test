...
$valid_formats = array("jpg", "png", "gif", "bmp", "jpeg","GIF","JPG","PNG"); //add the formats you want to upload
		...
		if(strlen($name)) //check if the file is selected or cancelled after pressing the browse button. 
		{
			...
			{
			if($size < 299888) // check if the file size is more than 2 mb
			{
			...
			if(move_uploaded_file($tmp, $path.$actual_image_name)) //check the path if it is fine
				{
					...
					echo "<img src='tools/uploads/".$actual_image_name."'  class='preview'> <input type='hidden' name='actual_image_name' id='actual_image_name' value='$actual_image_name' />";
				}
			else
				{
				echo "failed";
